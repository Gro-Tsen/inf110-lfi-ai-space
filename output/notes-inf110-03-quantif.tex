...
\review{La suite des diapositives consacrée à l'égalité et aux schémas
d'axiomes doit être intégrée ici dans une passe ultérieure.  Le contenu
source disponible pour ce brouillon s'interrompt avant la présentation
de ces éléments ; il ne faut pas inventer leurs règles ou leur notation
sans les reprendre des diapositives.}

\subsection{Curry--Howard pour la logique du premier ordre}

\review{Cette sous-section est réservée pour la reprise systématique de
la correspondance de Curry--Howard en logique du premier ordre.  Les
notions nécessaires ont été introduites ci-dessus pour $\forall$ et
$\exists$, mais les développements supplémentaires devront suivre
exactement les diapositives source.}